...
interface TodoApiService {
    ...
    @POST("todos")
    suspend fun postTodo(@Header("Authorization") authToken: String, @Body request: RequestTodo): ResponseMessage<ResponseTodoAdd?>

    @GET("todos/{todoId}")
    suspend fun getTodoById(@Header("Authorization") authToken: String, @Path("todoId") todoId: String): ResponseMessage<ResponseTodo?>

    @PUT("todos/{todoId}")
    suspend fun putTodo(@Header("Authorization") authToken: String, @Path("todoId") todoId: String, @Body request: RequestTodo): ResponseMessage<String?>

    @Multipart
    @PUT("todos/{todoId}/cover")
    suspend fun putTodoCover(@Header("Authorization") authToken: String, @Path("todoId") todoId: String, @Part file: MultipartBody.Part): ResponseMessage<String?>

    @DELETE("todos/{todoId}")
    suspend fun deleteTodo(@Header("Authorization") authToken: String, @Path("todoId") todoId: String): ResponseMessage<String?>
}